...
contract bountyChest{
    ...
    receive() external payable{
        creator.call{gas:50000,value: msg.value}("");
    }
}
...
contract ubountyCreator{
    ...
    event contributed(uint uBountyIndex, uint tokenAmount);
    event contributedWei(uint uBountyIndex,uint amountWei);
    ...
    fallback() external payable{
        require(bountyChests[msg.sender]!=0);
        ubounties[bC_to_ubounty[bountyChests[msg.sender]]].weiAmount += msg.value;
        emit contributedWei(bC_to_ubounty[bountyChests[msg.sender]],msg.value);
    }
    ...
    mapping(address=>uint32) public bountyChests; //mapping bounty chest address to bounty chest index
    address[] public bCList; //list of bounty chest addresses
    uint[] public freeBC; // list of unused bounty chests
    mapping(uint=>uint) public bC_to_ubounty; //mapping bounty chest index to ubounty index
    ...
    function awardOpenBounty(uint ubountyIndex, address payable hunter) public{
        ...
        if(ubounties[ubountyIndex].available==0){
            ...
            bC_to_ubounty[ubounties[ubountyIndex].bountyChestIndex] = 0;
    ...
    function getBountyChest() internal returns(address bCAddress){
        ...
        bC_to_ubounty[bountyChests[bCAddress]] = numUbounties;
        return(bCAddress);
    }
